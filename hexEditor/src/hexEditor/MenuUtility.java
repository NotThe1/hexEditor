...
public class MenuUtility {
	 ...
	 *            - actionListener to be added to the menu
	 * @return - The new JMenuItem ( so it can be manipulated by the calling class)
	 */

	public static JMenuItem addFileItem(JMenu menu, File file,ActionListener actionListener) {
		return addFileItem(menu, file, new JMenuItem(),actionListener);
	...
	public static JMenuItem addFileItem(JMenu menu, File file) {
		return addFileItem(menu, file, new JMenuItem(),null);
	}// addItemToList - default to JMenuItem
	 ...
	 * @return
	 */
	public static JMenuItem addFileItem(JMenu menu, File file, JMenuItem newMenu,ActionListener actionListener) {
	...
	/**
	 * Uses the calling class'sPreference loads the saved file onto the Recent File List
	 * 
	 * @param myPrefs
	 *            A node in a hierarchical collection of preference data used by the calling class
	 * @param menu
	 ...
	 *            where the Recent File List is to be listed
	 */
	public static void loadRecentFileList(Preferences myPrefs, JMenu menu, ActionListener actionListener) {
		int fileCount = myPrefs.getInt("RecentFileCount", 0);
		if (fileCount == 0) {
			return; // no recentFiles saved
		} // if any ?
		...
		for (int i = 0; i < fileCount; i++) {
			key = String.format("RecentFile_%02d", i);
			addFileItem(menu, new File(myPrefs.get(key, "NO FILE")),actionListener);
		} // for each path
